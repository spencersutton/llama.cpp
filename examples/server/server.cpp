...
struct llama_server_context {
  ...
  completion_token_output nextToken() {
...

    {
      ...
      llama_sample_repetition_penalty(ctx, &candidates_p, last_n_tokens.data() + last_n_tokens.size() - last_n_repeat,
      ...
      if (temp <= 0) {
        ...
        if (mirostat == 1) {
          ...
          result.tok =
              llama_sample_token_mirostat(ctx, &candidates_p, mirostat_tau, mirostat_eta, mirostat_m, &mirostat_mu);
        } else if (mirostat == 2) {
          static float mirostat_mu = 2.0f * mirostat_tau;
          llama_sample_temperature(ctx, &candidates_p, temp);
...
static void server_print_usage(const char *argv0, const gpt_params &params, const server_params &sparams) {
  ...
  fprintf(stderr,
          "                        how to split tensors across multiple GPUs, comma-separated list of proportions, "
          "e.g. 3,1\n");
  fprintf(stderr,
          "                        how to split tensors across multiple GPUs, comma-separated list of proportions, "
          "e.g. 3,1\n");
...
static json format_generation_settings(llama_server_context &llama) {
  ...
  const bool ignore_eos =
      eos_bias != llama.params.logit_bias.end() && eos_bias->second < 0.0f && std::isinf(eos_bias->second);
...
static json format_tokenizer_response(const std::vector<llama_token> &tokens) { return json{{"tokens", tokens}}; }
    ...
    if (!llama.stream) {
      ...
      while (llama.has_next_token) {
        ...
        const std::string token_text =
            token_with_probs.tok == -1 ? "" : llama_token_to_str(llama.ctx, token_with_probs.tok);
      ...
      const auto chunked_content_provider = [&](size_t, DataSink &sink) {
        ...
        while (llama.has_next_token) {
          ...
          const json data = llama.has_next_token ? format_partial_response(llama, to_send, probs_output)
