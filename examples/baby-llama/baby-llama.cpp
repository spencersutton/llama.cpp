...
struct ggml_tensor* randomize_tensor(struct ggml_tensor* tensor, int ndims, const int64_t ne[], float fmin,
                                     float fmax) {
  switch (ndims) {
    case 1:
    ...
    case 4:
      for (int i3 = 0; i3 < ne[3]; i3++) {
        for (int i2 = 0; i2 < ne[2]; i2++) {
          for (int i1 = 0; i1 < ne[1]; i1++) {
            for (int i0 = 0; i0 < ne[0]; i0++) {
              ((float*)tensor->data)[i3 * ne[2] * ne[1] * ne[0] + i2 * ne[1] * ne[0] + i1 * ne[0] + i0] =
...
struct ggml_tensor* randomize_tensor_normal(struct ggml_tensor* tensor, int ndims, const int64_t ne[],
...
struct llama_hparams {
  ...
  bool operator!=(const llama_hparams& other) const { return memcmp(this, &other, sizeof(llama_hparams)); }
};
...
void init_model(struct llama_model* model) {
  ...
  model->tok_embeddings =
      ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_embd, n_vocab);   // ("tok_embeddings.weight", {n_embd, n_vocab});
  model->norm = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, n_embd);  // ("norm.weight",           {n_embd});
  model->output =
      ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_embd, n_vocab);  // ("output.weight",         {n_embd, n_vocab});
  ...
  for (uint32_t i = 0; i < n_layer; ++i) {
    ...
    layer.attention_norm =
        ggml_new_tensor_1d(ctx, GGML_TYPE_F32, n_embd);  // (layers_i + ".attention_norm.weight", {n_embd});

    layer.wq = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_embd,
                                  n_embd);  // (layers_i + ".attention.wq.weight", {n_embd, n_embd});
    layer.wk = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_embd,
                                  n_embd);  // (layers_i + ".attention.wk.weight", {n_embd, n_embd});
    layer.wv = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_embd,
                                  n_embd);  // (layers_i + ".attention.wv.weight", {n_embd, n_embd});
    layer.wo = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_embd,
    ...
    layer.w1 = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_embd,
                                  n_ff);  // (layers_i + ".feed_forward.w1.weight", {n_embd,   n_ff});
    layer.w2 = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_ff,
                                  n_embd);  // (layers_i + ".feed_forward.w2.weight", {  n_ff,   n_embd});
    layer.w3 = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_embd,
...
void init_model_lora(struct llama_model_lora* model) {
  ...
  model->norm = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, n_embd);  // ("norm.weight",           {n_embd});
  model->outputa =
      ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_lora, n_vocab);  // ("output.weight",         {n_embd, n_vocab});
  model->outputb =
      ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_embd, n_lora);  // ("output.weight",         {n_embd, n_vocab});
  ...
  for (uint32_t i = 0; i < n_layer; ++i) {
    ...
    layer.wqa = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_lora,
                                   n_embd);  // (layers_i + ".attention.wq.weight", {n_embd, n_embd});
    layer.wqb = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_embd,
                                   n_lora);  // (layers_i + ".attention.wq.weight", {n_embd, n_embd});
    layer.wka = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_lora,
                                   n_embd);  // (layers_i + ".attention.wk.weight", {n_embd, n_embd});
    layer.wkb = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_embd,
                                   n_lora);  // (layers_i + ".attention.wk.weight", {n_embd, n_embd});
    layer.wva = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_lora,
                                   n_embd);  // (layers_i + ".attention.wv.weight", {n_embd, n_embd});
    layer.wvb = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_embd,
                                   n_lora);  // (layers_i + ".attention.wv.weight", {n_embd, n_embd});
    layer.woa = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_lora,
                                   n_embd);  // (layers_i + ".attention.wo.weight", {n_embd, n_embd});
    layer.wob = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, n_embd,
...
struct ggml_tensor* forward(struct llama_model* model, struct llama_kv_cache* cache, struct ggml_context* ctx0,
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    // self-attention
    {
      ...
      // store key and value to memory
      {
        ...
        kc = ggml_set_1d(ctx0, kc, ggml_reshape_1d(ctx0, Kcur, n_embd * N),
...
struct ggml_tensor* forward_batch(struct llama_model* model, struct llama_kv_cache* cache, struct ggml_context* ctx0,
                                  struct ggml_cgraph* gf, struct ggml_tensor* tokens_input, const int n_tokens,
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    // self-attention
    {
      ...
      // store key and value to memory
      {
        ...
        kc = ggml_set_2d(ctx0, kc, ggml_reshape_2d(ctx0, Kcur, n_embd * N, n_batch),
                         ggml_element_size(kc) * n_embd * n_ctx,
                         (ggml_element_size(kc) * n_embd) * (il * n_batch * n_ctx + n_past));
        vc = ggml_set_2d(ctx0, vc, ggml_reshape_2d(ctx0, Vcur, N * n_embd, n_batch),
...
struct ggml_tensor* forward_lora(struct llama_model_lora* model, struct llama_kv_cache* cache,
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    // self-attention
    {
      ...
      struct ggml_tensor* Qcur =
          ggml_rope(ctx0,
                    ggml_reshape_3d(
                        ctx0, ggml_mul_mat(ctx0, model->layers[il].wqa, ggml_mul_mat(ctx0, model->layers[il].wqb, cur)),
                        n_embd / n_head, n_head, N),
                    n_past, n_rot, 0, 0);
      struct ggml_tensor* Kcur =
          ggml_rope(ctx0,
                    ggml_reshape_3d(
                        ctx0, ggml_mul_mat(ctx0, model->layers[il].wka, ggml_mul_mat(ctx0, model->layers[il].wkb, cur)),
                        n_embd / n_head, n_head, N),
                    n_past, n_rot, 0, 0);
      ...
      // store key and value to memory
      {
        ...
        // Vcur shape [n_embd, N, 1, 1]
        struct ggml_tensor* Vcur = ggml_cont(
            ctx0, ggml_transpose(ctx0, ggml_reshape_2d(ctx0,
                                                       ggml_mul_mat(ctx0, model->layers[il].wva,
...
void sample_softmax_batch(struct ggml_context* ctx, struct ggml_tensor* logits, struct ggml_tensor* probs,
                          struct ggml_tensor* best_samples) {
  assert(best_samples->n_dims == 2);
  assert(logits->n_dims == 3);
  assert(probs->n_dims == 3);
  int n_tokens = best_samples->ne[0];
  int n_batch = best_samples->ne[1];
  int n_vocab = logits->ne[0];
  assert(n_tokens == logits->ne[1]);
  assert(n_batch == logits->ne[2]);
  assert(n_vocab == probs->ne[0]);
  assert(n_tokens == probs->ne[1]);
  assert(n_batch == probs->ne[2]);
  ...
  for (int k = 0; k < n_batch; ++k) {
    ...
    struct ggml_tensor* logits_k =
        ggml_view_2d(ctx, logits, logits->ne[0], logits->ne[1], logits->nb[1], k * logits->nb[2]);
...
void get_example_targets_batch(struct ggml_context* ctx, int example_id, struct ggml_tensor* tokens_input,
                               struct ggml_tensor* targets) {
  assert(tokens_input->n_dims == 2);
  assert(targets->n_dims == 3);
  int n_tokens = tokens_input->ne[0];
  int n_batch = tokens_input->ne[1];
  assert(n_tokens == targets->ne[1]);
  assert(n_batch == targets->ne[2]);
  ...
  for (int k = 0; k < n_batch; ++k) {
    ...
    struct ggml_tensor* targets_k =
        ggml_view_2d(ctx, targets, targets->ne[0], targets->ne[1], targets->nb[1], k * targets->nb[2]);
...
struct ggml_tensor* cross_entropy_loss(struct ggml_context* ctx, struct ggml_tensor* a, struct ggml_tensor* b) {
  ...
  return ggml_sum(ctx, ggml_neg(ctx, ggml_sum_rows(ctx, ggml_mul(ctx, ggml_soft_max(ctx, a),
