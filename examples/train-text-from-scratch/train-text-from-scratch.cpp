...
void init_random_normal_distribution(struct random_normal_distribution *rnd, int seed, float mean, float std, float min,
                                     float max) {
...
float frand_normal(struct random_normal_distribution *rnd) { return fclamp(rnd->rd(rnd->gen), rnd->min, rnd->max); }

float frand_uniform(struct random_uniform_distribution *rnd) { return rnd->rd(rnd->gen); }
...
struct my_llama_hparams {
  ...
  bool operator!=(const my_llama_hparams &other) const { return memcmp(this, &other, sizeof(my_llama_hparams)); }
};
...
struct ggml_tensor *forward(struct my_llama_model *model, struct my_llama_kv_cache *cache, struct ggml_context *ctx0,
                            struct ggml_cgraph *gf, struct ggml_tensor *tokens_input, const int n_tokens,
                            const int n_past) {
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    // self-attention
    {
      ...
      struct ggml_tensor *Qcur = ggml_rope_inplace(
          ctx0, ggml_reshape_3d(ctx0, ggml_mul_mat(ctx0, model->layers[il].wq, cur), n_embd / n_head, n_head, N),
          n_past, n_rot, 0, 0);
      struct ggml_tensor *Kcur = ggml_rope_inplace(
          ctx0, ggml_reshape_3d(ctx0, ggml_mul_mat(ctx0, model->layers[il].wk, cur), n_embd / n_head, n_head, N),
          n_past, n_rot, 0, 0);
      ...
      // store key and value to memory
      {
        ...
        struct ggml_tensor *Vcur = ggml_cont(
            ctx0,
            ggml_transpose(ctx0, ggml_reshape_2d(ctx0, ggml_mul_mat(ctx0, model->layers[il].wv, cur), n_embd, N)));
        ...
        (ggml_element_size(kv_self.k)*n_embd)*(il*n_ctx + n_past)); struct ggml_tensor * v = ggml_view_2d(ctx0,
        kv_self.v, N, n_embd, (   n_ctx)*ggml_element_size(kv_self.v), (il*n_ctx)*ggml_element_size(kv_self.v)*n_embd +
        ...
        kc = ggml_set_1d_inplace(ctx0, kc, ggml_reshape_1d(ctx0, Kcur, n_embd * N),
                                 (ggml_element_size(kv_self.k) * n_embd) * (il * n_ctx + n_past));
        vc = ggml_set_2d_inplace(
      ...
      struct ggml_tensor *KQ_scaled = ggml_scale(ctx0, KQ, ggml_new_f32(ctx0, 1.0f / sqrtf(float(n_embd) / n_head)));
      ...
      struct ggml_tensor *V =
          ggml_view_3d(ctx0, vc, n_past + N, n_embd / n_head, n_head, n_ctx * ggml_element_size(vc),
...
struct ggml_tensor *forward_batch(struct my_llama_model *model, struct my_llama_kv_cache *cache,
                                  struct ggml_context *ctx0, struct ggml_cgraph *gf, struct ggml_tensor *tokens_input,
                                  const int n_tokens, const int n_past, const int n_batch) {
  const int N = n_tokens;
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    // self-attention
    {
      ...
      struct ggml_tensor *Kcur = ggml_rope_inplace(
          ctx0,
          ggml_reshape_4d(ctx0, ggml_mul_mat(ctx0, model->layers[il].wk, cur), n_embd / n_head, n_head, N, n_batch),
          n_past, n_rot, 0, 0);
      ...
      // store key and value to memory
      {
        ...
        struct ggml_tensor *Vcur = ggml_cont(
            ctx0,
            ggml_permute(ctx0, ggml_reshape_3d(ctx0, ggml_mul_mat(ctx0, model->layers[il].wv, cur), n_embd, N, n_batch),
                         1, 0, 2, 3));
        ...
        kc = ggml_set_2d_inplace(ctx0, kc, ggml_reshape_2d(ctx0, Kcur, n_embd * N, n_batch),
                                 ggml_element_size(kc) * n_embd * n_ctx,
                                 (ggml_element_size(kc) * n_embd) * (il * n_batch * n_ctx + n_past));
        vc = ggml_set_2d_inplace(ctx0, vc, ggml_reshape_2d(ctx0, Vcur, N * n_embd, n_batch),
      ...
      struct ggml_tensor *KQ_scaled =
          ggml_scale_inplace(ctx0, KQ, ggml_new_f32(ctx0, 1.0f / sqrtf(float(n_embd) / n_head)));
      ...
      struct ggml_tensor *V =
          ggml_view_4d(ctx0, vc, n_past + N, n_embd / n_head, n_head, n_batch, ggml_element_size(vc) * n_ctx,
...
struct ggml_tensor *forward_batch_wo_cache(struct my_llama_model *model, struct ggml_context *ctx0,
                                           struct ggml_cgraph *gf, struct ggml_tensor *tokens_input, const int n_tokens,
                                           const int n_batch) {
  const int n_past = 0;
  const int N = n_tokens;
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    // self-attention
    {
      ...
      struct ggml_tensor *Vcur =
          ggml_reshape_4d(ctx0, ggml_mul_mat(ctx0, cur, model->layers[il].wv), N, n_batch, n_embd / n_head, n_head);
      ...
      struct ggml_tensor *V = ggml_permute(ctx0, Vcur, 0, 3, 1, 2);
...
struct ggml_tensor *forward_batch_wo_cache_flash_attn(struct my_llama_model *model, struct ggml_context *ctx0,
                                                      struct ggml_cgraph *gf, struct ggml_tensor *tokens_input,
                                                      const int n_tokens, const int n_batch) {
  const int n_past = 0;
  const int N = n_tokens;
...
struct ggml_tensor *expand(struct ggml_cgraph *g, struct ggml_tensor *t) {
  ...
  assert(g->n_nodes < GGML_MAX_NODES);
...
void graph_set_leafs_grads(struct ggml_cgraph *g) {
  ...
  for (int i = 0; i < g->n_nodes; ++i) {
    ...
    if (is_leaf) {
      assert(g->n_leafs < GGML_MAX_NODES);
...
struct ggml_tensor *forward_batch_wo_cache_flash_attn_train(
    struct my_llama_model *model, struct ggml_context *ctx0, struct ggml_cgraph *gf, struct ggml_cgraph *gb,
    struct ggml_tensor **logits, struct ggml_tensor *tokens_input, struct ggml_tensor *targets, void *compute_buf_0,
    void *compute_buf_1, size_t size_buf_0, size_t size_buf_1, const int n_tokens, const int n_batch) {
  ...
  size_t buf_size[2] = {size_buf_0, size_buf_1};
  void *buf_data[2] = {compute_buf_0, compute_buf_1};
  auto use_buf = [ctx0, &last_buf, &buf_offs, &buf_size, &buf_data](int buf) {
    size_t last_offs = 0;
    last_offs = ggml_set_scratch(ctx0, {
                                           0,
                                           0,
                                           nullptr,
                                       });
  ...
  struct ggml_tensor *t00 = ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, N * n_batch);
  assert_shape_1d(t00, N * n_batch);
  memcpy(t00->data, tokens_input->data, ggml_element_size(t00) * N * n_batch);

  use_buf(-1);

  struct ggml_tensor *t01 = expand(gf, ggml_get_rows(ctx0, model->tok_embeddings, t00));
  assert_shape_2d(t01, n_embd, N * n_batch);

  // need to remember these for the backward pass
  std::vector<struct ggml_tensor *> t02L;
  t02L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t03L;
  t03L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t04L;
  t04L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t05L;
  t05L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t06L;
  t06L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t07L;
  t07L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t08L;
  t08L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t09L;
  t09L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t10L;
  t10L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t11L;
  t11L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t12L;
  t12L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t13L;
  t13L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t14L;
  t14L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t15L;
  t15L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t16L;
  t16L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t17L;
  t17L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t18L;
  t18L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t19L;
  t19L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t20L;
  t20L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t21L;
  t21L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t22L;
  t22L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t23L;
  t23L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t24L;
  t24L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t25L;
  t25L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t26L;
  t26L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t27L;
  t27L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t28L;
  t28L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t29L;
  t29L.resize(n_layer, NULL);
  std::vector<struct ggml_tensor *> t30L;
  t30L.resize(n_layer, NULL);
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    use_buf(-1);
    struct ggml_tensor *t02 = expand(gf, ggml_rms_norm(ctx0, cur));
    assert_shape_2d(t02, n_embd, N * n_batch);
    use_buf(0);
    struct ggml_tensor *t03 = expand(gf, ggml_repeat(ctx0, layer.attention_norm, t02));
    assert_shape_2d(t03, n_embd, N * n_batch);
    use_buf(-1);
    struct ggml_tensor *t04 = expand(gf, ggml_mul(ctx0, t02, t03));
    assert_shape_2d(t04, n_embd, N * n_batch);
    use_buf(-1);
    struct ggml_tensor *t05 = expand(gf, ggml_mul_mat(ctx0, layer.wq, t04));
    assert_shape_2d(t05, n_embd, N * n_batch);
    use_buf(-1);
    struct ggml_tensor *t06 = expand(gf, ggml_reshape_4d(ctx0, t05, n_embd / n_head, n_head, N, n_batch));
    assert_shape_4d(t06, n_embd / n_head, n_head, N, n_batch);
    use_buf(-1);
    struct ggml_tensor *t07 = expand(gf, ggml_rope_inplace(ctx0, t06, n_past, n_rot, rope_mode, 0));
    assert_shape_4d(t07, n_embd / n_head, n_head, N, n_batch);
    use_buf(-1);
    struct ggml_tensor *t08 = expand(gf, ggml_mul_mat(ctx0, layer.wk, t04));
    assert_shape_2d(t08, n_embd, N * n_batch);
    use_buf(-1);
    struct ggml_tensor *t09 = expand(gf, ggml_reshape_4d(ctx0, t08, n_embd / n_head, n_head, N, n_batch));
    assert_shape_4d(t09, n_embd / n_head, n_head, N, n_batch);
    use_buf(-1);
    struct ggml_tensor *t10 = expand(gf, ggml_rope_inplace(ctx0, t09, n_past, n_rot, rope_mode, 0));
    assert_shape_4d(t10, n_embd / n_head, n_head, N, n_batch);
    use_buf(-1);
    struct ggml_tensor *t11 = expand(gf, ggml_mul_mat(ctx0, t04, layer.wv));
    assert_shape_2d(t11, N * n_batch, n_embd);
    use_buf(-1);
    struct ggml_tensor *t12 = expand(gf, ggml_reshape_4d(ctx0, t11, N, n_batch, n_embd / n_head, n_head));
    assert_shape_4d(t12, N, n_batch, n_embd / n_head, n_head);
    use_buf(-1);
    struct ggml_tensor *t13 = expand(gf, ggml_permute(ctx0, t07, 0, 2, 1, 3));
    assert_shape_4d(t13, n_embd / n_head, N, n_head, n_batch);
    use_buf(-1);
    struct ggml_tensor *t14 = expand(gf, ggml_permute(ctx0, t10, 0, 2, 1, 3));
    assert_shape_4d(t14, n_embd / n_head, N, n_head, n_batch);
    use_buf(-1);
    struct ggml_tensor *t15 = expand(gf, ggml_permute(ctx0, t12, 0, 3, 1, 2));
    assert_shape_4d(t15, N, n_embd / n_head, n_head, n_batch);
    use_buf(-1);
    struct ggml_tensor *t16 = expand(gf, ggml_flash_attn(ctx0, t13, t14, t15, true));
    assert_shape_4d(t16, n_embd / n_head, N, n_head, n_batch);
    use_buf(0);
    struct ggml_tensor *t17 = expand(gf, ggml_permute(ctx0, t16, 0, 2, 1, 3));
    assert_shape_4d(t17, n_embd / n_head, n_head, N, n_batch);
    use_buf(-1);
    struct ggml_tensor *t18 = expand(gf, ggml_cont(ctx0, t17));
    assert_shape_4d(t18, n_embd / n_head, n_head, N, n_batch);
    use_buf(-1);
    struct ggml_tensor *t19 = expand(gf, ggml_reshape_2d(ctx0, t18, n_embd, N * n_batch));
    assert_shape_2d(t19, n_embd, N * n_batch);
    use_buf(0);
    struct ggml_tensor *t20 = expand(gf, ggml_mul_mat(ctx0, layer.wo, t19));
    assert_shape_2d(t20, n_embd, N * n_batch);
    use_buf(-1);
    struct ggml_tensor *t21 = expand(gf, ggml_add(ctx0, t20, cur));
    assert_shape_2d(t21, n_embd, N * n_batch);
    use_buf(-1);
    struct ggml_tensor *t22 = expand(gf, ggml_rms_norm(ctx0, t21));
    assert_shape_2d(t22, n_embd, N * n_batch);
    use_buf(0);
    struct ggml_tensor *t23 = expand(gf, ggml_repeat(ctx0, layer.ffn_norm, t22));
    assert_shape_2d(t23, n_embd, N * n_batch);
    use_buf(-1);
    struct ggml_tensor *t24 = expand(gf, ggml_mul(ctx0, t23, t22));
    assert_shape_2d(t24, n_embd, N * n_batch);
    use_buf(-1);
    struct ggml_tensor *t25 = expand(gf, ggml_mul_mat(ctx0, layer.w3, t24));
    assert_shape_2d(t25, n_ff, N * n_batch);
    use_buf(-1);
    struct ggml_tensor *t26 = expand(gf, ggml_mul_mat(ctx0, layer.w1, t24));
    assert_shape_2d(t26, n_ff, N * n_batch);
    use_buf(-1);
    struct ggml_tensor *t27 = expand(gf, ggml_silu(ctx0, t26));
    assert_shape_2d(t27, n_ff, N * n_batch);
    use_buf(-1);
    struct ggml_tensor *t28 = expand(gf, ggml_mul(ctx0, t27, t25));
    assert_shape_2d(t28, n_ff, N * n_batch);
    use_buf(0);
    struct ggml_tensor *t29 = expand(gf, ggml_mul_mat(ctx0, layer.w2, t28));
    assert_shape_2d(t29, n_embd, N * n_batch);
    use_buf(-1);
    struct ggml_tensor *t30 = expand(gf, ggml_add(ctx0, t21, t29));
  ...
  clr_buf(0);
  use_buf(0);
  struct ggml_tensor *t31 = expand(gf, ggml_rms_norm(ctx0, cur));
  assert_shape_2d(t31, n_embd, N * n_batch);
  struct ggml_tensor *t32 = expand(gf, ggml_repeat(ctx0, model->norm, t31));
  assert_shape_2d(t32, n_embd, N * n_batch);
  struct ggml_tensor *t33 = expand(gf, ggml_mul(ctx0, t32, t31));
  assert_shape_2d(t33, n_embd, N * n_batch);
  use_buf(-1);
  struct ggml_tensor *t34 = expand(gf, ggml_mul_mat(ctx0, model->output, t33));
  assert_shape_2d(t34, n_vocab, N * n_batch);
  struct ggml_tensor *t35 = expand(gf, ggml_reshape_3d(ctx0, t34, n_vocab, N, n_batch));
  assert_shape_3d(t35, n_vocab, N, n_batch);
  struct ggml_tensor *t36 = expand(gf, ggml_cross_entropy_loss(ctx0, t35, targets));
  assert_shape_1d(t36, 1);

  {
      /*
      tok_embeddings                                                        | grad_tok_embeddings =
      ggml_get_rows_back(grad_t01, t00) L0_att_norm                                                           |
      grad_L0_att_norm    = ggml_repeat_back(grad_t03L0, L0_att_norm.shape) L0_wq | grad_L0_wq          =
      ggml_out_prod(t04L0, grad_t05L0) L0_wk                                                                 |
      grad_L0_wk          = ggml_out_prod(t04L0, grad_t08L0) L0_wv | grad_L0_wv          = ggml_out_prod(t04L0,
      ggml_transpose(grad_t11L0)) L0_wo                                                                 | grad_L0_wo =
      ggml_out_prod(t19L0, grad_t20L0) L0_ffn_norm                                                           |
      grad_L0_ffn_norm    = ggml_repeat_back(grad_t23L0, L0_ffn_norm.shape) L0_w1 | grad_L0_w1          =
      ggml_out_prod(t24L0, grad_t26L0) L0_w2                                                                 |
      grad_L0_w2          = ggml_out_prod(t28L0, grad_t29L0) L0_w3 | grad_L0_w3          = ggml_out_prod(t24L0,
      grad_t25L0) L1_att_norm                                                           | grad_L1_att_norm    =
      ggml_repeat_back(grad_t03L1, L1_att_norm.shape) L1_wq | grad_L1_wq          = ggml_out_prod(t04L1, grad_t05L1)
      L1_wk                                                                 | grad_L1_wk          = ggml_out_prod(t04L1,
      grad_t08L1) L1_wv                                                                 | grad_L1_wv          =
      ggml_out_prod(t04L1, ggml_transpose(grad_t11L1)) L1_wo | grad_L1_wo          = ggml_out_prod(t19L1, grad_t20L1)
      L1_ffn_norm                                                           | grad_L1_ffn_norm    =
      ggml_repeat_back(grad_t23L1, L1_ffn_norm.shape) L1_w1 | grad_L1_w1          = ggml_out_prod(t24L1, grad_t26L1)
      L1_w2                                                                 | grad_L1_w2          = ggml_out_prod(t28L1,
      grad_t29L1) L1_w3                                                                 | grad_L1_w3          =
      ggml_out_prod(t24L1, grad_t25L1) norm                                                                  | grad_norm
      = ggml_repeat_back(grad_t32, norm.shape) output                                                                |
      grad_output         = ggml_out_prod(t33, grad_t34)
                                                                            |
      t01 = ggml_get_rows(tok_embeddings, t00)                              | grad_t01   = grad_t21L0 +
      ggml_rms_norm_back(t01, grad_t02L0) for layer:                                                            |
      ...
      t04L0*= ggml_mul          (t02L0, t03L0)                              | grad_t04L0 = ggml_out_prod(L0_wv,
      grad_t11L0) + ggml_out_prod(L0_wk, ggml_transpose(grad_t08L0)) + ggml_out_prod(L0_wq, ggml_transpose(grad_t05L0))
      t05L0 = ggml_mul_mat      (L0_wq, t04L0)                              | grad_t05L0 = ggml_reshape(grad_t06L0,
      t05L0_shape) t06L0 = ggml_reshape_4d   (t05L0, n_embd/n_head, n_head, N, n_batch)  | grad_t06L0 =
      ggml_rope_back(grad_t07L0) t07L0 = ggml_rope_inplace (t06L0)                                     | grad_t07L0 =
      ggml_permute_back(grad_t13L0, 0, 2, 1, 3) = ggml_permute(grad_t13L0, 0, 2, 1, 3) t08L0 = ggml_mul_mat      (L0_wk,
      ...
      (t09L0)                                     | grad_t10L0 = ggml_permute_back(grad_t14L0, 0, 2, 1, 3) =
      ggml_permute(grad_t14L0, 0, 2, 1, 3) t11L0 = ggml_mul_mat      (t04L0, L0_wv)                              |
      grad_t11L0 = ggml_reshape(grad_t12L0, t11L0_shape) t12L0 = ggml_reshape_4d   (t11L0, N, n_batch, n_embd/n_head,
      n_head)  | grad_t12L0 = ggml_permute_back(grad_t15L0, 0, 3, 1, 2) = ggml_permute(grad_t15L0, 0, 2, 3, 1) t13L0*=
      ggml_permute      (t07L0, 0, 2, 1, 3)                         | grad_t13L0 = view__q(ggml_flash_attn_back(t13L0,
      t14L0, t15L0, grad_t16L0)) t14L0*= ggml_permute      (t10L0, 0, 2, 1, 3)                         | grad_t14L0 =
      view__k(ggml_flash_attn_back(t13L0, t14L0, t15L0, grad_t16L0)) t15L0*= ggml_permute      (t12L0, 0, 3, 1, 2) |
      grad_t15L0 = view__v(ggml_flash_attn_back(t13L0, t14L0, t15L0, grad_t16L0)) t16L0 = ggml_flash_attn   (t13L0,
      t14L0, t15L0)                       | grad_t16L0 = ggml_permute_back(grad_t17L0, 0, 2, 1, 3) =
      ggml_permute(grad_t17L0, 0, 2, 1, 3) t17L0 = ggml_permute      (t16L0, 0, 2, 1, 3)                         |
      grad_t17L0 = grad_t18L0 t18L0 = ggml_cont         (t17L0)                                     | grad_t18L0 =
      ggml_reshape(grad_t19L0, t18L0_shape) t19L0*= ggml_reshape_2d   (t18L0, n_embd, N*n_batch)                  |
      grad_t19L0 = ggml_out_prod(L0_wo, ggml_transpose(grad_t20L0)) t20L0 = ggml_mul_mat      (L0_wo, t19L0) |
      grad_t20L0 = grad_t21L0 t21L0*= ggml_add          (t20L0, t01)                                | grad_t21L0 =
      grad_t30L0 + ggml_rms_norm_back(t21L0, grad_t22L0) t22L0*= ggml_rms_norm     (t21L0) | grad_t22L0 =
      ggml_mul(grad_t24L0, t23L0) t23L0 = ggml_repeat       (L0_ffn_norm, t22L0_shape)                  | grad_t23L0 =
      ggml_mul(grad_t24L0, t22L0) t24L0*= ggml_mul          (t23L0, t22L0)                              | grad_t24L0 =
      ggml_out_prod(L0_w1, ggml_transpose(grad_t26L0)) + ggml_out_prod(L0_w3, ggml_transpose(grad_t25L0)) t25L0*=
      ggml_mul_mat      (L0_w3, t24L0)                              | grad_t25L0 = ggml_mul(grad_t28L0, t27L0) t26L0*=
      ggml_mul_mat      (L0_w1, t24L0)                              | grad_t26L0 = ggml_silu_back(t26L0, grad_t27L0)
      t27L0*= ggml_silu         (t26L0)                                     | grad_t27L0 = ggml_mul(grad_t28L0, t25L0)
      t28L0*= ggml_mul          (t27L0, t25L0)                              | grad_t28L0 = ggml_out_prod(L0_w2,
      ggml_transpose(grad_t29L0)) t29L0 = ggml_mul_mat      (L0_w2, t28L0)                              | grad_t29L0 =
      grad_t30L0 t30L0*= ggml_add          (t21L0, t29L0)                              | grad_t30L0 =
      ggml_rms_norm_back(t30L0, grad_t02L1) + grad_t21L1
                                                                            ^
      ...
      t04L1*= ggml_mul          (t02L1, t03L1)                              | grad_t04L1 = ggml_out_prod(L1_wv,
      grad_t11L1) + ggml_out_prod(L1_wk, ggml_transpose(grad_t08L1)) + ggml_out_prod(L1_wq, ggml_transpose(grad_t05L1))
      t05L1 = ggml_mul_mat      (L1_wq, t04L1)                              | grad_t05L1 = ggml_reshape(grad_t06L1,
      t05L1_shape) t06L1 = ggml_reshape_4d   (t05L1, n_embd/n_head, n_head, N, n_batch)  | grad_t06L1 =
      ggml_rope_back(grad_t07L1) t07L1 = ggml_rope_inplace (t06L1)                                     | grad_t07L1 =
      ggml_permute_back(grad_t13L1, 0, 2, 1, 3) = ggml_permute(grad_t13L1, 0, 2, 1, 3) t08L1 = ggml_mul_mat      (L1_wk,
      t04L1)                              | grad_t08L1 = ggml_reshape(grad_t09L1, t08L1_shape) t09L1 = ggml_reshape_4d
      (t08L1, n_embd/n_head, n_head, N, n_batch)  | grad_t09L1 = ggml_rope_back(grad_t10L1) t10L1 = ggml_rope_inplace
      (t09L1)                                     | grad_t10L1 = ggml_permute_back(grad_t14L1, 0, 2, 1, 3) =
      ggml_permute(grad_t14L1, 0, 2, 1, 3) t11L1 = ggml_mul_mat      (t04L1, L1_wv)                              |
      grad_t11L1 = ggml_reshape(grad_t12L1, t11L1_shape) t12L1 = ggml_reshape_4d   (t11L1, N, n_batch, n_embd/n_head,
      n_head)  | grad_t12L1 = ggml_permute_back(grad_t15L1, 0, 3, 1, 2) = ggml_permute(grad_t15L1, 0, 2, 3, 1) t13L1*=
      ggml_permute      (t07L1, 0, 2, 1, 3)                         | grad_t13L1 = view__q(ggml_flash_attn_back(t13L1,
      t14L1, t15L1, grad_t16L1)) t14L1*= ggml_permute      (t10L1, 0, 2, 1, 3)                         | grad_t14L1 =
      view__k(ggml_flash_attn_back(t13L1, t14L1, t15L1, grad_t16L1)) t15L1*= ggml_permute      (t12L1, 0, 3, 1, 2) |
      grad_t15L1 = view__v(ggml_flash_attn_back(t13L1, t14L1, t15L1, grad_t16L1)) t16L1 = ggml_flash_attn   (t13L1,
      t14L1, t15L1)                       | grad_t16L1 = ggml_permute_back(grad_t17L1, 0, 2, 1, 3) =
      ggml_permute(grad_t17L1, 0, 2, 1, 3) t17L1 = ggml_permute      (t16L1, 0, 2, 1, 3)                         |
      grad_t17L1 = grad_t18L1 t18L1 = ggml_cont         (t17L1)                                     | grad_t18L1 =
      ggml_reshape(grad_t19L1, t18L1_shape) t19L1*= ggml_reshape_2d   (t18L1, n_embd, N*n_batch)                  |
      grad_t19L1 = ggml_out_prod(L1_wo, ggml_transpose(grad_t20L1)) t20L1 = ggml_mul_mat      (L1_wo, t19L1) |
      grad_t20L1 = grad_t21L1 t21L1*= ggml_add          (t20L1, t30L0)                              | grad_t21L1 =
      grad_t30L1 + ggml_rms_norm_back(t21L1, grad_t22L1) t22L1*= ggml_rms_norm     (t21L1) | grad_t22L1 =
      ggml_mul(grad_t24L1, t23L1) t23L1 = ggml_repeat       (L1_ffn_norm, t22L1_shape)                  | grad_t23L1 =
      ggml_mul(grad_t24L1, t22L1) t24L1*= ggml_mul          (t23L1, t22L1)                              | grad_t24L1 =
      ggml_out_prod(L1_w1, ggml_transpose(grad_t26L1)) + ggml_out_prod(L1_w3, ggml_transpose(grad_t25L1)) t25L1*=
      ggml_mul_mat      (L1_w3, t24L1)                              | grad_t25L1 = ggml_mul(grad_t28L1, t27L1) t26L1*=
      ggml_mul_mat      (L1_w1, t24L1)                              | grad_t26L1 = ggml_silu_back(t26L1, grad_t27L1)
      t27L1*= ggml_silu         (t26L1)                                     | grad_t27L1 = ggml_mul(grad_t28L1, t25L1)
      t28L1*= ggml_mul          (t27L1, t25L1)                              | grad_t28L1 = ggml_out_prod(L1_w2,
      ggml_transpose(grad_t29L1)) t29L1 = ggml_mul_mat      (L1_w2, t28L1)                              | grad_t29L1 =
      grad_t30L1 t30L1*= ggml_add          (t21L1, t29L1)                              | grad_t30L1 =
      ggml_rms_norm_back(t30L1, grad_t31)
                                                                            ^
      t31   = ggml_rms_norm     (t30L1)                                     | grad_t31   = ggml_mul(grad_t33, t32)
      t32   = ggml_repeat       (norm, t31.shape)                           | grad_t32   = ggml_mul(grad_t33, t31)
      t33   = ggml_mul          (t32, t31)                                  | grad_t33   = ggml_out_prod(output,
      ggml_transpose(grad_t34)) t34   = ggml_mul_mat      (output, t33)                               | grad_t34   =
      ggml_reshape(grad_t35, t34.shape) t35   = ggml_reshape_3d   (t34, n_vocab, N, n_batch)                  | grad_t35
      = ggml_cross_entropy_loss_back(t35, targets, grad_t36) t36   = ggml_cross_entropy_loss(t35, targets) | grad_t36 =
      1 (optimizer) tensors marked with * need to be stored until grad computation tensors during grad computation are
      all temporary
      */
  }
  ...
  use_buf(0);
  t35->grad = expand(gb, ggml_cross_entropy_loss_back(ctx0, t35, targets, t36->grad));
  assert_shape_3d(t35->grad, n_vocab, N, n_batch);
  t34->grad = expand(gb, ggml_reshape_2d(ctx0, t35->grad, n_vocab, N * n_batch));
  assert_shape_2d(t34->grad, n_vocab, N * n_batch);
  t33->grad = expand(gb, ggml_out_prod(ctx0, model->output, ggml_transpose(ctx0, t34->grad)));
  assert_shape_2d(t33->grad, n_embd, N * n_batch);
  t32->grad = expand(gb, ggml_mul(ctx0, t33->grad, t31));
  assert_shape_2d(t32->grad, n_embd, N * n_batch);

  use_buf(-1);

  model->norm->grad = expand(gb, add_or_set(model->norm->grad, ggml_repeat_back(ctx0, t32->grad, model->norm)));
  assert_shape_1d(model->norm->grad, n_embd);
  model->output->grad = expand(gb, add_or_set(model->output->grad, ggml_out_prod(ctx0, t33, t34->grad)));
  assert_shape_2d(model->output->grad, n_embd, n_vocab);

  clr_buf(1);
  use_buf(1);
  t31->grad = expand(gb, ggml_mul(ctx0, t33->grad, t32));
  assert_shape_2d(t31->grad, n_embd, N * n_batch);
  ...
  for (int k = 0; k < n_layer; ++k) {
    ...
    clr_buf(0);
    use_buf(0);
    t30->grad = expand(gb, ggml_rms_norm_back(ctx0, t30, back_layer_inp->grad));
    assert_shape_2d(t30->grad, n_embd, N * n_batch);
    if (grad_layer_inp) {
      t30->grad = expand(gb, ggml_add(ctx0, t30->grad, grad_layer_inp->grad));
      assert_shape_2d(t30->grad, n_embd, N * n_batch);
    }
    clr_buf(1);
    t29->grad = t30->grad;
    assert_shape_2d(t29->grad, n_embd, N * n_batch);
    t28->grad = expand(gb, ggml_out_prod(ctx0, layer.w2, ggml_transpose(ctx0, t29->grad)));
    assert_shape_2d(t28->grad, n_ff, N * n_batch);
    t27->grad = expand(gb, ggml_mul(ctx0, t28->grad, t25));
    assert_shape_2d(t27->grad, n_ff, N * n_batch);
    t26->grad = expand(gb, ggml_silu_back(ctx0, t26, t27->grad));
    assert_shape_2d(t26->grad, n_ff, N * n_batch);
    t25->grad = expand(gb, ggml_mul(ctx0, t28->grad, t27));
    assert_shape_2d(t25->grad, n_ff, N * n_batch);
    t24->grad = expand(gb, ggml_add_inplace(ctx0, ggml_out_prod(ctx0, layer.w1, ggml_transpose(ctx0, t26->grad)),
                                            ggml_out_prod(ctx0, layer.w3, ggml_transpose(ctx0, t25->grad))));
    assert_shape_2d(t24->grad, n_embd, N * n_batch);
    t23->grad = expand(gb, ggml_mul(ctx0, t24->grad, t22));
    assert_shape_2d(t23->grad, n_embd, N * n_batch);
    t22->grad = expand(gb, ggml_mul(ctx0, t24->grad, ggml_repeat(ctx0, layer.ffn_norm, t24->grad)));
    assert_shape_2d(t22->grad, n_embd, N * n_batch);
    use_buf(1);
    t21->grad = expand(gb, ggml_add(ctx0, t30->grad, ggml_rms_norm_back(ctx0, t21, t22->grad)));
    assert_shape_2d(t21->grad, n_embd, N * n_batch);
    grad_layer_inp = t21;
    use_buf(0);
    t20->grad = t21->grad;
    assert_shape_2d(t20->grad, n_embd, N * n_batch);
    t19->grad = expand(gb, ggml_out_prod(ctx0, layer.wo, ggml_transpose(ctx0, t20->grad)));
    assert_shape_2d(t19->grad, n_embd, N * n_batch);
    t18->grad = expand(gb, ggml_reshape_4d(ctx0, t19->grad, n_embd / n_head, n_head, N, n_batch));
    assert_shape_4d(t18->grad, n_embd / n_head, n_head, N, n_batch);
    t17->grad = t18->grad;
    assert_shape_4d(t17->grad, n_embd / n_head, n_head, N, n_batch);
    t16->grad = expand(gb, ggml_permute(ctx0, t17->grad, 0, 2, 1, 3));
    assert_shape_4d(t16->grad, n_embd / n_head, N, n_head, n_batch);
    struct ggml_tensor *flash_attn = expand(gb, ggml_flash_attn_back(ctx0, t13, t14, t15, t16->grad, true));
    assert_shape_4d(flash_attn, n_embd / n_head, N * 3, n_head, n_batch);
    t15->grad = expand(gb, view__v(flash_attn));
    assert_shape_4d(t15->grad, N, n_embd / n_head, n_head, n_batch);
    t14->grad = expand(gb, view__k(flash_attn));
    assert_shape_4d(t14->grad, n_embd / n_head, N, n_head, n_batch);
    t13->grad = expand(gb, view__q(flash_attn));
    assert_shape_4d(t13->grad, n_embd / n_head, N, n_head, n_batch);
    t12->grad = expand(gb, ggml_permute(ctx0, t15->grad, 0, 2, 3, 1));
    assert_shape_4d(t12->grad, N, n_batch, n_embd / n_head, n_head);
    t11->grad = expand(gb, ggml_reshape_2d(ctx0, ggml_cont(ctx0, t12->grad), N * n_batch, n_embd));
    assert_shape_2d(t11->grad, N * n_batch, n_embd);
    t10->grad = expand(gb, ggml_permute(ctx0, t14->grad, 0, 2, 1, 3));
    assert_shape_4d(t10->grad, n_embd / n_head, n_head, N, n_batch);
    t09->grad = expand(gb, ggml_rope_back(ctx0, t10->grad, n_past, n_rot, rope_mode));
    assert_shape_4d(t09->grad, n_embd / n_head, n_head, N, n_batch);
    t08->grad = expand(gb, ggml_reshape_2d(ctx0, t09->grad, n_embd, N * n_batch));
    assert_shape_2d(t08->grad, n_embd, N * n_batch);
    t07->grad = expand(gb, ggml_permute(ctx0, t13->grad, 0, 2, 1, 3));
    assert_shape_4d(t07->grad, n_embd / n_head, n_head, N, n_batch);
    t06->grad = expand(gb, ggml_rope_back(ctx0, t07->grad, n_past, n_rot, rope_mode));
    assert_shape_4d(t06->grad, n_embd / n_head, n_head, N, n_batch);
    t05->grad = expand(gb, ggml_reshape_2d(ctx0, t06->grad, n_embd, N * n_batch));
    assert_shape_2d(t05->grad, n_embd, N * n_batch);
    t04->grad =
        expand(gb, ggml_add_inplace(ctx0,
                                    ggml_add_inplace(ctx0, ggml_out_prod(ctx0, layer.wv, t11->grad),
                                                     ggml_out_prod(ctx0, layer.wk, ggml_transpose(ctx0, t08->grad))),
                                    ggml_out_prod(ctx0, layer.wq, ggml_transpose(ctx0, t05->grad))));
    assert_shape_2d(t04->grad, n_embd, N * n_batch);
    t03->grad = expand(gb, ggml_mul(ctx0, t04->grad, t02));
    assert_shape_2d(t04->grad, n_embd, N * n_batch);
    use_buf(1);
    t02->grad = expand(gb, ggml_mul(ctx0, t04->grad, ggml_repeat(ctx0, layer.attention_norm, t02)));
    assert_shape_2d(t02->grad, n_embd, N * n_batch);
    back_layer_inp = t02;
    // use_buf(0);

    use_buf(-1);
    layer.attention_norm->grad =
        expand(gb, add_or_set(layer.attention_norm->grad, ggml_repeat_back(ctx0, t03->grad, layer.attention_norm)));
    assert_shape_1d(layer.attention_norm->grad, n_embd);
    layer.wq->grad = expand(gb, add_or_set(layer.wq->grad, ggml_out_prod(ctx0, t04, t05->grad)));
    assert_shape_2d(layer.wq->grad, n_embd, n_embd);
    layer.wk->grad = expand(gb, add_or_set(layer.wk->grad, ggml_out_prod(ctx0, t04, t08->grad)));
    assert_shape_2d(layer.wk->grad, n_embd, n_embd);
    layer.wv->grad = expand(gb, add_or_set(layer.wv->grad, ggml_out_prod(ctx0, t04, ggml_transpose(ctx0, t11->grad))));
    assert_shape_2d(layer.wv->grad, n_embd, n_embd);
    layer.wo->grad = expand(gb, add_or_set(layer.wo->grad, ggml_out_prod(ctx0, t19, t20->grad)));
    assert_shape_2d(layer.wo->grad, n_embd, n_embd);
    layer.ffn_norm->grad =
        expand(gb, add_or_set(layer.ffn_norm->grad, ggml_repeat_back(ctx0, t23->grad, layer.ffn_norm)));
    assert_shape_1d(layer.ffn_norm->grad, n_embd);
    layer.w1->grad = expand(gb, add_or_set(layer.w1->grad, ggml_out_prod(ctx0, t24, t26->grad)));
    assert_shape_2d(layer.w1->grad, n_embd, n_ff);
    layer.w2->grad = expand(gb, add_or_set(layer.w2->grad, ggml_out_prod(ctx0, t28, t29->grad)));
    assert_shape_2d(layer.w2->grad, n_ff, n_embd);
    layer.w3->grad = expand(gb, add_or_set(layer.w3->grad, ggml_out_prod(ctx0, t24, t25->grad)));
    assert_shape_2d(layer.w3->grad, n_embd, n_ff);
    // use_buf(0);
  }
  clr_buf(0);
  use_buf(0);
  t01->grad =
      expand(gb, ggml_add_inplace(ctx0, grad_layer_inp->grad, ggml_rms_norm_back(ctx0, t01, back_layer_inp->grad)));
  assert_shape_2d(t01->grad, n_embd, N * n_batch);
  use_buf(-1);
  model->tok_embeddings->grad = expand(gb, ggml_get_rows_back(ctx0, t01->grad, t00, model->tok_embeddings));
  assert_shape_2d(model->tok_embeddings->grad, n_embd, n_vocab);
  // clr_buf(1);
  // clr_buf(0);
...
void print_token(struct llama_context *ctx, llama_token token) { printf("%s", llama_token_to_str(ctx, token)); }
...
void get_example_targets(const int *train_samples, size_t n_train_samples, const llama_token *train_data,
                         size_t n_train_data, int example_id, struct ggml_tensor *tokens_input,
                         struct ggml_tensor *target_logits, struct ggml_tensor *target_probs) {
  int n_tokens = tokens_input->ne[0];
  int n_vocab = target_logits->ne[0];

  size_t sample = train_samples[example_id % n_train_samples];
  assert(sample + n_tokens - 1 < n_train_data);
...
void get_example_targets_batch(struct llama_context * /*lctx*/, const int *train_samples, size_t n_train_samples,
                               const llama_token *train_data, size_t n_train_data, int example_id,
                               struct ggml_tensor *tokens_input, struct ggml_tensor *target_logits,
                               struct ggml_tensor *target_probs) {
  assert(tokens_input->n_dims == 2);
  assert(target_logits->n_dims == 3);
  assert(target_probs->n_dims == 3);
  int n_vocab = target_logits->ne[0];
  int n_tokens = tokens_input->ne[0];
  int n_batch = tokens_input->ne[1];
  assert(n_tokens == target_logits->ne[1]);
  assert(n_batch == target_logits->ne[2]);
  assert(n_vocab == target_probs->ne[0]);
  assert(n_tokens == target_probs->ne[1]);
  assert(n_batch == target_probs->ne[2]);
...
void lshift_examples(struct ggml_tensor *tokens_input, struct ggml_tensor *target_logits,
                     struct ggml_tensor *target_probs, int n_shift) {
  int n_tokens = tokens_input->ne[0];
  int n_vocab = target_logits->ne[0];
...
void shuffle_ints(int *begin, int *end) {
  ...
  std::sort(begin, end, [&vals](int a, int b) { return vals.at(a) < vals.at(b); });
}
...
llama_token sample(struct my_llama_sampler *sampler, float *logits, const llama_token *last_tokens, int n_last_tokens) {
  ...
  llama_sample_repetition_penalty(ctx, candidates_p, last_tokens + n_last_tokens - n_last, n_last,
                                  params.repeat_penalty);
  llama_sample_frequency_and_presence_penalties(ctx, candidates_p, last_tokens + n_last_tokens - n_last, n_last,
                                                params.alpha_frequency, params.alpha_presence);
  ...
  if (params.temp <= 0) {
    ...
    if (params.mirostat == 1) {
      ...
      token = llama_sample_token_mirostat(ctx, candidates_p, params.mirostat_tau, params.mirostat_eta, mirostat_m,
                                          &sampler->mirostat_mu);
    } else if (params.mirostat == 2) {
      llama_sample_temperature(ctx, candidates_p, params.temp);
      token = llama_sample_token_mirostat_v2(ctx, candidates_p, params.mirostat_tau, params.mirostat_eta,
                                             &sampler->mirostat_mu);
    } else {
...
void set_logits_masked(struct ggml_tensor *logits, std::vector<bool> &mask, float value) {
  assert(logits->ne[0] == (int64_t)mask.size());
...
void write_tensor(struct llama_file *file, struct ggml_tensor *tensor) {
  ...
  uint32_t ne[4] = {(uint32_t)tensor->ne[0], (uint32_t)tensor->ne[1], (uint32_t)tensor->ne[2], (uint32_t)tensor->ne[3]};
  file->write_u32(nd);
...
void read_tensor(struct llama_file *file, struct ggml_tensor *tensor) {
  ...
  assert(nd == tensor->n_dims);

  uint32_t name_len = file->read_u32();
  enum ggml_type type = (enum ggml_type)file->read_u32();
  assert(type == tensor->type);

  uint32_t ne[4];
  file->read_raw(ne, sizeof(ne[0]) * nd);
  for (int i = 0; i < nd; ++i) {
    assert(ne[i] == tensor->ne[i]);
  }

  std::string name = file->read_string(name_len);
  assert(strncmp(ggml_get_name(tensor), name.c_str(), sizeof(tensor->name) - 1) == 0);
...
void train_print_usage(int /*argc*/, char **argv, const struct train_params *params) {
  ...
  fprintf(stderr,
          "  --mult N                   Mult size used for new models, influences feedforward size. (default %d)\n",
          params->n_mult);
  ...
  fprintf(stderr,
          "  --lbfgs-iter N             Maximum number of LBFGS optimization iterations for each batch (default %d)\n",
          params->lbfgs_n_iter);
  fprintf(stderr,
          "  --adam-iter N              Maximum number of Adam optimization iterations for each batch (default %d)\n",
          params->adam_n_iter);
  fprintf(stderr, "  --adam-alpha N             Adam learning rate alpha (default %f)\n", params->adam_alpha);
  fprintf(stderr,
          "  --adam-decay N             AdamW weight decay. Values greater zero enable AdamW instead of regular Adam. "
          "(default %f)\n",
          params->adam_decay);
...
int main(int argc, char **argv) {
  ...
  struct llama_vocab vocab;
  {
    ...
    assert(n_vocab == llama_n_vocab(lctx));
  ...
  assert(n_tokens < (int)train_tokens.size());
  ...
  for (int i = 0; i < (int)train_samples.size(); ++i) {
    assert(train_samples[i] + n_tokens - 1 < (int)train_tokens.size());
  }
  ...
  for (int ex = 0; ex < params.n_examples; ++ex) {
    ...
    // struct ggml_tensor * after_opt_probs        = ggml_new_tensor_3d(ctx0, GGML_TYPE_F32, n_vocab,  n_tokens,
    // n_batch);
    ...
    get_example_targets_batch(lctx, train_samples.data(), train_samples.size(), train_tokens.data(),
                              train_tokens.size(), ex, tokens_input, target_logits, target_probs);
    ...
    if (params.use_scratch) {
      loss = forward_batch_wo_cache_flash_attn_train(&model, ctx0, gf, gb, &logits, tokens_input, target_probs,
                                                     compute_buf_0, compute_buf_1, size_buf_0, size_buf_1, n_tokens,
                                                     n_batch);
    ...
    opt->params.adam.sched = (opt->iter < params.warmup)
                                 ...
                                 : cosine_decay_restart(params.cos_decay_steps, params.cos_decay_alpha,
                                                        opt->iter - params.warmup, params.cos_decay_restart);
    ...
    if (params.print_details_interval > 0 && ex % params.print_details_interval == 0) {
      ...
      for (int i = 0; i < n_batch; ++i) {
        ...
        for (int k = 0; k < n_tokens; ++k) {
          int32_t token = sample(&sampler, (float *)((char *)logits->data + i * logits->nb[2] + k * logits->nb[1]),
                                 (llama_token *)((char *)tokens_input->data + i * tokens_input->nb[1]), k);
          *((int32_t *)((char *)after_opt_best_samples->data + i * after_opt_best_samples->nb[1] +
                        k * after_opt_best_samples->nb[0])) = token;
        }
      }
...

  {
    ...
    get_example_targets(train_samples.data(), train_samples.size(), train_tokens.data(), train_tokens.size(),
                        rand() % train_samples.size(), tokens_input, target_logits, target_probs);
    ...
    for (int i = 0; i < n_gen; ++i) {
      ...
      // set_logits_masked(logits, token_notavail, -1e9);
      int token = sample(&sampler, (float *)((char *)logits->data + (sample_ctx - 1) * logits->nb[1]),
                         (llama_token *)tokens_input->data, sample_ctx - 1);
