...
void gpt_print_usage(int /*argc*/, char **argv, const gpt_params &params) {
  ...
  fprintf(stderr,
          "  --cfg-smooth-factor N smooth factor between old and new logits (default: %f, 1.0 = no smoothing)\n",
  ...
  fprintf(stderr,
          "                        if run without this previously, it is recommended to drop the system page cache "
          "before using this\n");
...
std::tuple<struct llama_model *, struct llama_context *> llama_init_from_gpt_params(const gpt_params &params) {
  ...
  if (!params.lora_adapter.empty()) {
    int err =
        llama_model_apply_lora_from_file(model, params.lora_adapter.c_str(),
...
void pop_cursor(console_state &con_st) { putc('\b', con_st.out); }

int estimateWidth(char32_t codepoint) { return wcwidth(codepoint); }
...
void replace_last(console_state &con_st, char ch) { fprintf(con_st.out, "\b%c", ch); }
