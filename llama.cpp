...
static const std::map<e_model, size_t> &MEM_REQ_SCRATCH0() {
  static std::map<e_model, size_t> k_sizes = {
      {MODEL_3B, 256ull * MB},  {MODEL_7B, 512ull * MB},   {MODEL_13B, 512ull * MB},
      {MODEL_30B, 512ull * MB}, {MODEL_65B, 1024ull * MB},
  };
  return k_sizes;
}
...
static const std::map<e_model, size_t> &MEM_REQ_KV_SELF() {
  static std::map<e_model, size_t> k_sizes = {
      {MODEL_3B, 682ull * MB},   {MODEL_7B, 1026ull * MB},  {MODEL_13B, 1608ull * MB},
      {MODEL_30B, 3124ull * MB}, {MODEL_65B, 5120ull * MB},
  };
  return k_sizes;
}
...
static const std::map<e_model, size_t> &MEM_REQ_EVAL() {
  static std::map<e_model, size_t> k_sizes = {
      {MODEL_3B, 512ull * MB},   {MODEL_7B, 768ull * MB},   {MODEL_13B, 1024ull * MB},
      {MODEL_30B, 1280ull * MB}, {MODEL_65B, 1536ull * MB},
  };
  return k_sizes;
}
...
static const std::map<e_model, size_t> &VRAM_REQ_SCRATCH_BASE() {
  static std::map<e_model, size_t> k_sizes = {
      {MODEL_3B, 512ull * kB},  {MODEL_7B, 512ull * kB},   {MODEL_13B, 640ull * kB},
      {MODEL_30B, 768ull * kB}, {MODEL_65B, 1536ull * kB},
  };
  return k_sizes;
}
...
static const std::map<e_model, size_t> &VRAM_REQ_SCRATCH_PER_CONTEXT() {
  static std::map<e_model, size_t> k_sizes = {
      {MODEL_3B, 128ull}, {MODEL_7B, 128ull}, {MODEL_13B, 160ull}, {MODEL_30B, 208ull}, {MODEL_65B, 416ull},
  };
  return k_sizes;
}
...
struct llama_context {
  llama_context(const llama_model &model, const llama_vocab &vocab)
      : model(model), vocab(vocab), t_load_us(model.t_load_us), t_start_us(model.t_start_us) {}
#ifdef GGML_USE_METAL
  ~llama_context() {
    if (ctx_metal) {
  ...
  size_t get_buf_max_mem(int i) const { return buf_max_size[i]; }
};
...
static T checked_mul(T a, T b) {
  T ret = a * b;
  if (a != 0 && ret / a != b) {
    throw std::runtime_error(format("overflow multiplying %llu * %llu", (unsigned long long)a, (unsigned long long)b));
  }
  return ret;
}
...
struct llama_file_loader {
  ...
  void read_magic() {
    ...
    throw std::runtime_error(
        format("unknown (magic, version) combination: %08x, %08x; is this really a GGML file?", magic, version));
  }
  void read_hparams() {
...
struct llama_model_loader {
  ...
  struct ggml_tensor *get_tensor(const std::string &name, const std::vector<uint32_t> &ne, ggml_backend backend) {
    ...
    if (it == tensors_map.name_to_idx.end()) {
      throw std::runtime_error(
          std::runtime_error(format("llama.cpp: tensor '%s' is missing from model", name.c_str())));
    }
    llama_load_tensor &lt = tensors_map.tensors.at(it->second);
    if (lt.ne != ne) {
      throw std::runtime_error(format("llama.cpp: tensor '%s' has wrong shape; expected %s, got %s", name.c_str(),
                                      llama_format_tensor_shape(ne).c_str(), llama_format_tensor_shape(lt.ne).c_str()));
    }
...
static bool kv_cache_init(const struct llama_hparams &hparams, struct llama_kv_cache &cache, ggml_type wtype, int n_ctx,
                          int n_gpu_layers) {
...
static void llama_model_load_internal(const std::string &fname, llama_model &model, llama_vocab &vocab, int n_ctx,
                                      ...
                                      bool has_lora, bool vocab_only, llama_progress_callback progress_callback,
  ...
  if (file_version < LLAMA_FILE_VERSION_GGJT_V2) {
    if (hparams.ftype != LLAMA_FTYPE_ALL_F32 && hparams.ftype != LLAMA_FTYPE_MOSTLY_F16 &&
        hparams.ftype != LLAMA_FTYPE_MOSTLY_Q8_0) {
      throw std::runtime_error(
          format("this format is no longer supported (see https://github.com/ggerganov/llama.cpp/pull/1405)"));
    }
  }

  if (file_version < LLAMA_FILE_VERSION_GGJT_V3) {
    if (hparams.ftype == LLAMA_FTYPE_MOSTLY_Q4_0 || hparams.ftype == LLAMA_FTYPE_MOSTLY_Q4_1 ||
        hparams.ftype == LLAMA_FTYPE_MOSTLY_Q8_0) {
      throw std::runtime_error(
          format("this format is no longer supported (see https://github.com/ggerganov/llama.cpp/pull/1508)"));
    }
  }
...
#define LLAMA_BACKEND_OFFLOAD_SPLIT GGML_BACKEND_CPU
  ...
  size_t vram_scratch = 0;
  {
    ...
    for (uint32_t i = 0; i < n_layer; ++i) {
      ...
      if (backend == GGML_BACKEND_GPU) {
        vram_weights += ggml_nbytes(layer.attention_norm) + ggml_nbytes(layer.wq) + ggml_nbytes(layer.wk) +
  ...
  // print memory requirements
  {
    ...
    const size_t mem_required = ctx_size + mmapped_size - vram_weights +  // weights in VRAM not in memory
...
static bool llama_model_load(const std::string &fname, llama_model &model, llama_vocab &vocab, int n_ctx, int n_batch,
                             int n_gpu_layers, int main_gpu, float *tensor_split, bool low_vram, ggml_type memory_type,
  ...
  try {
    llama_model_load_internal(fname, model, vocab, n_ctx, n_batch, n_gpu_layers, main_gpu, tensor_split, low_vram,
                              memory_type, use_mmap, use_mlock, has_lora, vocab_only, progress_callback,
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    // self-attention
    {
      ...
      struct ggml_tensor *Kcur =
          ggml_rope_inplace(ctx0, ggml_reshape_3d(ctx0, tmpk, n_embd / n_head, n_head, N), n_past, n_rot, 0, 0);
      offload_func_kq(Kcur);
      ggml_set_name(Kcur, "Kcur");

      struct ggml_tensor *Qcur =
          ggml_rope_inplace(ctx0, ggml_reshape_3d(ctx0, tmpq, n_embd / n_head, n_head, N), n_past, n_rot, 0, 0);
      ...
      // store key and value to memory
      {
        ...
        struct ggml_tensor *k =
            ggml_view_1d(ctx0, kv_self.k, N * n_embd, (ggml_element_size(kv_self.k) * n_embd) * (il * n_ctx + n_past));
        offload_func_kq(k);
        ggml_set_name(k, "k");

        struct ggml_tensor *v =
            ggml_view_2d(ctx0, kv_self.v, N, n_embd, (n_ctx)*ggml_element_size(kv_self.v),
      ...
      struct ggml_tensor *V = ggml_view_3d(
          ctx0, kv_self.v, n_past + N, n_embd / n_head, n_head, n_ctx * ggml_element_size(kv_self.v),
          n_ctx * ggml_element_size(kv_self.v) * n_embd / n_head, il * n_ctx * ggml_element_size(kv_self.v) * n_embd);
      offload_func_v(V);
      ggml_set_name(V, "V");
...
static void llama_convert_tensor_internal(const llama_load_tensor &tensor, llama_buffer &output, const int nelements,
                                          const int nthread) {
  ...
  for (auto tnum = 0, in_buff_offs = 0, out_buff_offs = 0; tnum < nthread; tnum++) {
    ...
    workers.push_back(
        std::thread(compute, tensor.type, tensor.data + in_buff_offs, f32_output + out_buff_offs, thr_elems));
...
static void llama_model_quantize_internal(const std::string &fname_inp, const std::string &fname_out,
  ...
  std::unique_ptr<llama_model_loader> model_loader(
      new llama_model_loader(fname_inp, /*use_mmap*/ false, /*has_lora*/ false));
  ...
  for (llama_load_tensor &tensor : model_loader->tensors_map.tensors) {
    ...
    printf("[%4zu/%4zu] %36s - %16s, type = %6s, ", ++idx, model_loader->tensors_map.tensors.size(),
           tensor.name.c_str(), llama_format_tensor_shape(tensor.ne).c_str(), ggml_type_name(tensor.type));
    ...
    if (!quantize) {
      ...
      if (tensor.name == "output.weight") {
        ...
        // else if (ftype == LLAMA_FTYPE_MOSTLY_Q4_K_S && i_feed_forward_w2 < n_feed_forward_w2/8) new_type =
        // GGML_TYPE_Q6_K;
        ++i_feed_forward_w2;
...
struct llama_model *llama_load_model_from_file(const char *path_model, struct llama_context_params params) {
  ...
  if (!llama_model_load(path_model, *model, model->vocab, params.n_ctx, params.n_batch, params.n_gpu_layers,
                        params.main_gpu, params.tensor_split, params.low_vram, memory_type, params.use_mmap,
                        params.use_mlock, params.has_lora, params.vocab_only, params.progress_callback,
...
#define LLAMA_METAL_CHECK_BUF(result)                        \
  if (!(result)) {                                           \
    ...
    LLAMA_METAL_CHECK_BUF(
        ggml_metal_add_buffer(ctx->ctx_metal, "eval", ctx->buf_compute.addr, ctx->buf_compute.size, 0));
    LLAMA_METAL_CHECK_BUF(ggml_metal_add_buffer(ctx->ctx_metal, "kv", ctx->kv_self.buf.addr, ctx->kv_self.buf.size, 0));

    LLAMA_METAL_CHECK_BUF(
        ggml_metal_add_buffer(ctx->ctx_metal, "scr0", ctx->buf_scratch[0].addr, ctx->buf_scratch[0].size, 0));
    LLAMA_METAL_CHECK_BUF(
...
int llama_apply_lora_from_file_internal(const struct llama_model &model, const char *path_lora,
  ...
  if (path_base_model) {
    ...
    if (model_loader->use_mmap) {
      model_loader->mapping.reset(
          new llama_mmap(&model_loader->file_loader->file, /* prefetch */ 0, ggml_is_numa(), model_loader->has_lora));
    }
  }
  ...
  while (true) {
    ...
    if (lora_tensors.find(base_name + ".loraA") != lora_tensors.end() &&
      ...
      if (model_loader) {
        ...
        base_t =
            model_loader->get_tensor(base_name, {(uint32_t)dest_t->ne[0], (uint32_t)dest_t->ne[1]}, GGML_BACKEND_CPU);
        lt.data = (uint8_t *)lt.ggml_tensor->data;
...
int llama_apply_lora_from_file(struct llama_context *ctx, const char *path_lora, const char *path_base_model,
                               int n_threads) {
  try {
...
size_t llama_copy_state_data(struct llama_context *ctx, uint8_t *dst) {
  ...
  // copy kv cache
  {
    ...
    if (kv_size) {
      ...
      ggml_tensor *k3d =
          ggml_view_3d(cpy_ctx, kv_self.k, n_embd, kv_ntok, n_layer, elt_size * n_embd, elt_size * n_embd * n_ctx, 0);

      ggml_tensor *v3d =
          ggml_view_3d(cpy_ctx, kv_self.v, kv_ntok, n_embd, n_layer, elt_size * n_ctx, elt_size * n_ctx * n_embd, 0);
...
int llama_n_vocab(const struct llama_context *ctx) { return ctx->vocab.id_to_token.size(); }

int llama_n_ctx(const struct llama_context *ctx) { return ctx->model.hparams.n_ctx; }

int llama_n_embd(const struct llama_context *ctx) { return ctx->model.hparams.n_embd; }
