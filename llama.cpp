...
template <typename T> static T checked_mul(T a, T b) {
  T ret = a * b;
  if (a != 0 && ret / a != b) {
  ...
  void calc_ne() {
    ...
    for (const auto &shard : shards) {
      if (shard.ne != first_shard.ne) {
        throw format("inconsistent tensor shard shape in '%s': first was %s, "
                     "other was %s",
                     name.c_str(),
                     llama_format_tensor_shape(first_shard.ne).c_str(),
...
struct llama_file_loader {
  ...
  void read_magic() {
    ...
    if (magic == 'ggml' && version == 0) {
      ...
      throw format("unknown (magic, version) combination: %08x, %08x; is this "
                   "really a GGML file?",
                   magic, version);
    }
  }
...
struct llama_file_saver {
  ...
  void write_vocab() {
    if (any_file_loader->file_version == LLAMA_FILE_VERSION_GGML) {
      fprintf(stderr, "llama.cpp: WARNING: input is an old file that doesn't "
...
struct llama_model_loader {
  ...
  llama_model_loader(const std::string &fname_base, bool use_mmap,
    ...
    if (use_mmap && alignment_prevents_mmap()) {
      fprintf(stderr, "llama.cpp: can't use mmap because tensors are not "
                      "aligned; convert to new format to avoid this\n");
      use_mmap = false;
    }
  ...
  void load_all_data(llama_progress_callback progress_callback,
    ...
    for (llama_load_tensor &lt : tensors_map.tensors) {
      ...
      LLAMA_ASSERT(lt.ggml_tensor); // unused tensors should have been caught by
                                    // load_data already
...
static void llama_model_load_internal(const std::string &fname,
...

  {
    ...
    fprintf(stderr, "%s: ftype      = %u (%s)\n", __func__, hparams.ftype,
            llama_ftype_name(hparams.ftype));
    fprintf(stderr, "%s: n_ff       = %u\n", __func__, n_ff);
    fprintf(stderr, "%s: n_parts    = %zu\n", __func__,
            ml->file_loaders.size());
  ...
  // print memory requirements
  {
    ...
    const size_t mem_required =
        ctx_size + mmapped_size + MEM_REQ_SCRATCH0.at(model.type) +
        MEM_REQ_SCRATCH1.at(model.type) + MEM_REQ_EVAL.at(model.type);
  ...
  // prepare memory for the weights
  {
    ...
    model.tok_embeddings =
        ml->get_tensor("tok_embeddings.weight", {n_embd, n_vocab});
    ...
    for (uint32_t i = 0; i < n_layer; ++i) {
      ...
      layer.attention_norm =
          ml->get_tensor(layers_i + ".attention_norm.weight", {n_embd});

      layer.wq =
          ml->get_tensor(layers_i + ".attention.wq.weight", {n_embd, n_embd});
      layer.wk =
          ml->get_tensor(layers_i + ".attention.wk.weight", {n_embd, n_embd});
      layer.wv =
          ml->get_tensor(layers_i + ".attention.wv.weight", {n_embd, n_embd});
      layer.wo =
          ml->get_tensor(layers_i + ".attention.wo.weight", {n_embd, n_embd});

      layer.ffn_norm = ml->get_tensor(layers_i + ".ffn_norm.weight", {n_embd});

      layer.w1 =
          ml->get_tensor(layers_i + ".feed_forward.w1.weight", {n_embd, n_ff});
      layer.w2 =
          ml->get_tensor(layers_i + ".feed_forward.w2.weight", {n_ff, n_embd});
      layer.w3 =
          ml->get_tensor(layers_i + ".feed_forward.w3.weight", {n_embd, n_ff});
    }
  }
...
static bool llama_model_load(const std::string &fname, llama_context &lctx,
                             int n_ctx, ggml_type memory_type, bool use_mmap,
                             bool use_mlock, bool vocab_only,
                             llama_progress_callback progress_callback,
                             void *progress_callback_user_data) {
  try {
    llama_model_load_internal(fname, lctx, n_ctx, memory_type, use_mmap,
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    // self-attention
    {
      ...
      struct ggml_tensor *KQ_scaled = ggml_scale(
          ctx0, KQ, ggml_new_f32(ctx0, 1.0f / sqrtf(float(n_embd) / n_head)));
...
llama_tokenize(const llama_vocab &vocab, const std::string &text, bool bos) {
...
static void
sample_top_k(std::vector<std::pair<float, llama_vocab::id>> &logits_id,
             int top_k) {
  // find the top k tokens
  std::partial_sort(logits_id.begin(), logits_id.begin() + top_k,
                    logits_id.end(),
                    [](const std::pair<float, llama_vocab::id> &a,
                       const std::pair<float, llama_vocab::id> &b) {
...
static llama_vocab::id llama_sample_top_p_top_k(
    llama_context &lctx, const std::vector<llama_vocab::id> &last_n_tokens,
  ...
  //     printf("%d: '%s' %f\n", i,
  //     lctx.vocab.id_to_token.at(logits_id[i].second).tok.c_str(), probs[i]);
  // }
  // printf("\n\n");
  // exit(0);
...
static void llama_model_quantize_internal(const std::string &fname_inp,
  ...
  std::unique_ptr<llama_model_loader> model_loader(
      new llama_model_loader(fname_inp.c_str(), /*use_mmap*/ false,
                             /*vocab_only*/ false));
  ...
  for (llama_load_tensor &tensor : model_loader->tensors_map.tensors) {
    ...
    printf("[%zu/%zu] %36s - %s, type = %6s, ", ++idx,
    ...
    if (!quantize) {
      ...
      switch (new_type) {
      case GGML_TYPE_Q4_0: {
        new_size = ggml_quantize_q4_0(f32_data, new_data, nelements,
                                      (int)tensor.ne.at(0), hist_cur.data());
      } break;
      case GGML_TYPE_Q4_1: {
        new_size = ggml_quantize_q4_1(f32_data, new_data, nelements,
                                      (int)tensor.ne.at(0), hist_cur.data());
      } break;
      default:
        LLAMA_ASSERT(false);
...
void llama_set_kv_cache(struct llama_context *ctx, const uint8_t *kv_cache,
                        size_t n_size, int n_token_count) {
...
int llama_n_ctx(struct llama_context *ctx) { return ctx->model.hparams.n_ctx; }
...
llama_token llama_sample_top_p_top_k(llama_context *ctx,
                                     ...
                                     int last_n_tokens_size, int top_k,
                                     float top_p, float temp,
  ...
  // TODO: avoid this ...
  const auto last_n_tokens = std::vector<llama_token>(
      last_n_tokens_data, last_n_tokens_data + last_n_tokens_size);

  result = llama_sample_top_p_top_k(*ctx, last_n_tokens, top_k, top_p, temp,
                                    repeat_penalty);
...
void llama_print_timings(struct llama_context *ctx) {
  ...
  fprintf(stderr,
          "%s:        eval time = %8.2f ms / %5d runs   (%8.2f ms per run)\n",
