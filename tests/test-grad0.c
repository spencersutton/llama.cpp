...
struct ggml_tensor *get_random_tensor(struct ggml_context *ctx0, int ndims, int64_t ne[], float fmin, float fmax) {
    ...
    case 4:
      for (int i3 = 0; i3 < ne[3]; i3++) {
        for (int i2 = 0; i2 < ne[2]; i2++) {
          for (int i1 = 0; i1 < ne[1]; i1++) {
            for (int i0 = 0; i0 < ne[0]; i0++) {
              ((float *)result->data)[i3 * ne[2] * ne[1] * ne[0] + i2 * ne[1] * ne[0] + i1 * ne[0] + i0] =
...
struct ggml_tensor *get_random_tensor_int(struct ggml_context *ctx0, int ndims, int64_t ne[], int32_t imin,
                                          int32_t imax) {
    ...
    case 4:
      for (int i3 = 0; i3 < ne[3]; i3++) {
        for (int i2 = 0; i2 < ne[2]; i2++) {
          for (int i1 = 0; i1 < ne[1]; i1++) {
            for (int i0 = 0; i0 < ne[0]; i0++) {
              ((int32_t *)result->data)[i3 * ne[2] * ne[1] * ne[0] + i2 * ne[1] * ne[0] + i1 * ne[0] + i0] =
...
bool check_gradient(const char *op_name, struct ggml_context *ctx0, struct ggml_tensor *x[], struct ggml_tensor *f,
                    int ndims, int nargs, float eps, float max_error_abs, float max_error_rel) {
  ...
  for (int i = 0; i < nargs; ++i) {
    ...
    for (int k = 0; k < nelements; ++k) {
      ...
      if (error_abs > max_error_abs || error_rel > max_error_rel) {
        printf(
            "%s: ndims=%d, i=%d, k=%d, x0=%f, xm=%f, xp=%f, f0=%f, f1=%f, g0=%f, g1=%f, eps=%f, error_abs=%f, "
            "error_rel=%f\n",
...
// TODO: clean-up this ..
bool check_mat_mul(const struct ggml_tensor *y, const struct ggml_tensor *x0, const struct ggml_tensor *x1) {
  ...
  for (int iter = 0; iter < niter; ++iter) {
    ...
    // mul_mat
    {
      ...
      for (int ndims = 2; ndims <= 2; ++ndims) {
        ...
        GGML_PRINT_DEBUG("testing: mul_mat, [%lld, %lld] (%d) * [%lld, %lld] (%d)\n", x[1]->ne[0], x[1]->ne[1],
                         x[1]->n_dims, x[0]->ne[0], x[0]->ne[1], x[0]->n_dims);
    ...
    // acc 1d
    {
      ...
      for (int ndims = 1; ndims <= 4; ++ndims) {
        ...
        struct ggml_tensor *f =
            ggml_sum(ctx0, ggml_acc(ctx0, x[0], x[1], x[0]->nb[1], x[0]->nb[2], x[0]->nb[3], offset));
    ...
    // acc 3d
    {
      ...
      for (int ndims = 3; ndims <= 4; ++ndims) {
        ...
        while ((ne2[0] > ne[0]) || (ne2[1] > ne[1]) || (ne2[2] > ne[2]) ||
    ...
    // acc 4d
    {
      ...
      for (int ndims = 4; ndims <= 4; ++ndims) {
        ...
        get_random_dims(ne2, 4);
        while ((ne2[0] > ne[0]) || (ne2[1] > ne[1]) || (ne2[2] > ne[2]) || (ne2[3] > ne[3]) ||
               (ne2[0] * ne2[1] * ne2[2] * ne2[3] > ggml_nelements(x[0]))) {
          get_random_dims(ne2, 4);
        }
    ...
    // view_3d
    {
      ...
      for (int ndims = 1; ndims <= 4; ++ndims) {
        ...
        struct ggml_tensor *f =
            ggml_sum(ctx0, ggml_view_3d(ctx0, x[0], ne2[0], ne2[1], ne2[2], nb2[1], nb2[2], offset));
